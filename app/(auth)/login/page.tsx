    ...
    <div className="min-h-screen flex bg-[#FAFAFA] dark:bg-[#0a0a0f]">
      ...
      <div className="hidden lg:flex lg:w-[45%] bg-[#2A398D] relative overflow-hidden items-end p-12">
        ...
        <div className="relative z-10">
          ...
          <h1 className="font-display text-5xl text-white leading-tight mb-3">
            Copa Mundial<br />FIFA 2026
          </h1>
          <p className="font-body text-white/60 text-sm max-w-xs">
            Predice los resultados, compite con amigos y demuestra quién sabe más de fútbol.
          </p>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        ...
        >
          ...
          <h2 className="font-display text-2xl text-gray-900 dark:text-white mb-1">Bienvenido</h2>
          <p className="text-sm font-body text-gray-400 mb-8">Ingresa a tu cuenta para continuar</p>
          ...
          {(() => {
            ...
            if (errorParam) {
              if (errorParam === 'OAuthCallbackError') {
                friendlyError = 'Hubo un problema al iniciar sesión con Google. Por favor, inténtalo de nuevo.'
              } else if (errorParam === 'profile_creation_failed') {
                friendlyError = 'No se pudo crear el perfil de usuario. Por favor, vuelve a intentar iniciar sesión.'
              } else {
          ...
          <p className="text-center text-sm font-body text-gray-400 mt-6">
            ¿No tienes cuenta?{' '}
            <Link href={`/register${next ? `?next=${encodeURIComponent(next)}` : ''}`} className="text-[#2A398D] dark:text-blue-400 hover:underline font-medium">
              Crear cuenta
            </Link>
          </p>
        </motion.div>
