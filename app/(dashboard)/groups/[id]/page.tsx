...
export default async function GroupDetailPage({ params }: PageProps) {
  ...
  const [
    ...
    admin
      .from('room_members')
      .select('user_id, joined_at, payment_status, predicted_champion_id, predicted_goleador')
      .eq('room_id', id),
  ...
  const groupPredictions = GROUP_LETTERS.reduce(
    (acc, letter) => {
      acc[letter] =
        (allGroupPreds || []).find((p) => p.user_id === user.id && p.group_letter === letter) ||
        null
      return acc
    },
  ...
  const allMembersPredictions = memberUserIds.map((uid) => {
    ...
    const memberGroupPreds = GROUP_LETTERS.reduce(
      (acc, letter) => {
        acc[letter] =
          (allGroupPreds || []).find((p) => p.user_id === uid && p.group_letter === letter) || null
        return acc
      },
      {} as Record<GroupLetter, GroupPrediction | null>
    )
    ...
    for (const p of (allKnockoutPreds || []).filter((pp) => pp.user_id === uid)) {
      const matchNumber = (p.matches as unknown as { match_number: number })?.match_number
      if (matchNumber) {
        memberKnockoutPreds[matchNumber] = p.predicted_winner_id
